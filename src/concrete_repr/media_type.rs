...
pub static TEXT_TURTLE: Lazy<Mime> = Lazy::new(|| {
    "text/turtle".parse().unwrap()
});

pub static APPLICATION_N_TRIPLES: Lazy<Mime> = Lazy::new(|| {
    "application/n-triples".parse().unwrap()
});

pub static APPLICATION_N_QUADS: Lazy<Mime> = Lazy::new(|| {
    "application/n-quads".parse().unwrap()
});

pub static APPLICATION_RDF_XML: Lazy<Mime> = Lazy::new(|| {
    "application/rdf+xml".parse().unwrap()
});

pub static TEXT_OWL_MANCHESTER: Lazy<Mime> = Lazy::new(|| {
...
pub static APPLICATION_TRIG: Lazy<Mime> = Lazy::new(|| {
    "application/trig".parse().unwrap()
});
